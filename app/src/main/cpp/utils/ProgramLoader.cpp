...
ProgramLoader::ProgramLoader(const std::string& logTag, const std::string& vert_file_name,
                             const std::string& frag_file_name, AAssetManager *mgr)
{
    status = -1;
    if(mgr == nullptr)
    {
    ...
    if(!linked)
    {
        ...
        if(length > 0)
        {
            ...
            glGetProgramInfoLog(program_id, length, nullptr, info);
...
GLuint ProgramLoader::loadShaderFile(AAssetManager *mgr, const std::string& file_name, int shader_type)
{
    // 首先读取文件内容
    ...
    glShaderSource(shader, 1, reinterpret_cast<const GLchar *const *>(&src),nullptr);
