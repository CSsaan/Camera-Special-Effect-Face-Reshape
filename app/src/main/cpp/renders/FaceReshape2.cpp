...
//FBO显示关键点
#define FACERESHAPE2_VERY_SHADER_FILE "shaders/facereshape2/facereshape2_vert.glsl"
#define FACERESHAPE2_FRAG_SHADER_FILE "shaders/facereshape2/facereshape2_frag.glsl"
//普通2D显示的着色器（显示FBO结果）
#define D2_FACERESHAPE2_VERT_SHADER_FILE "shaders/facereshape2/2D_face_reshape2_vert.glsl"
#define D2_FACERESHAPE2_FRAG_SHADER_FILE "shaders/facereshape2/2D_face_reshape2_frag.glsl"

FaceReshape2::FaceReshape2(): BaseVideoRender(false,"","")
{
    // *创建程序*
    face_program = std::shared_ptr<ProgramLoader>(new ProgramLoader("FaceReshape2", FACERESHAPE2_VERY_SHADER_FILE, FACERESHAPE2_FRAG_SHADER_FILE, asset_mgr));
    my2D_program = std::shared_ptr<ProgramLoader>(new ProgramLoader("FaceReshape2_2D", D2_FACERESHAPE2_VERT_SHADER_FILE, D2_FACERESHAPE2_FRAG_SHADER_FILE, asset_mgr));
    //检查创建program
    if (face_program->status != 0 || my2D_program->status != 0) // status=-1时，创建program失败
    {
        LOGE("[FaceReshape2()]: create_program error!");
    }
    init_FaceReshape2_data();
}

FaceReshape2::~FaceReshape2()
{
    // *释放vao和vbo资源*
...
//*********************************************************************************************
void FaceReshape2::init_FaceReshape2_data()
{
    //FBO顶点数组
    float vert_fbo[] = {
            -1.0, 1.0, 0.0, 1.0, // 左上
...
//*********************************************************************************************
//读取BaseVideoRender的数据
void FaceReshape2::readBasePixel()
{
    if(img_w == 0 || img_h == 0) {
        return;
    }
...
//*********************************************************************************************
void FaceReshape2::render_FaceReshape2()
{
    // 要开启混色模式
    ...
    if(has_face == 1)
    {
        ...
        glUniform2fv(points_loc, 98, points);
    }
    //FBO离屏渲染(传入原图)
