...
//FBO显示关键点
#define BIGEYES_VERY_SHADER_FILE "shaders/BigEyes/BigEyes_vert.glsl"
#define BIGEYES_FRAG_SHADER_FILE "shaders/BigEyes/BigEyes_frag.glsl"
//普通2D显示的着色器（显示FBO结果）
#define D2_BIGEYES_VERT_SHADER_FILE "shaders/BigEyes/2D_BigEyes_vert.glsl"
#define D2_BIGEYES_FRAG_SHADER_FILE "shaders/BigEyes/2D_BigEyes_frag.glsl"

BigEyes::BigEyes(): BaseVideoRender(false,"","")
{
    // *创建程序*
    face_program = std::shared_ptr<ProgramLoader>(new ProgramLoader("BigEyes", BIGEYES_VERY_SHADER_FILE, BIGEYES_FRAG_SHADER_FILE, asset_mgr));
    my2D_program = std::shared_ptr<ProgramLoader>(new ProgramLoader("BigEyes_2D", D2_BIGEYES_VERT_SHADER_FILE, D2_BIGEYES_FRAG_SHADER_FILE, asset_mgr));
    //检查创建program
    if (face_program->status != 0 || my2D_program->status != 0) // status=-1时，创建program失败
    {
        LOGE("[BigEyes()]: create_program error!");
    }
    init_BigEyes_data();
}
...
//*********************************************************************************************
void BigEyes::init_BigEyes_data()
{
    //FBO顶点数组
    float vert_fbo[] = {
            -1.0, 1.0, 0.0, 1.0, // 左上
    ...
    Vstrength_loc = glGetUniformLocation(face_program->program_id, "u_ScaleRatio");
    strength_loc = glGetUniformLocation(face_program->program_id, "u_Radius");
