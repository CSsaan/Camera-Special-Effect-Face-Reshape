//
// Created by chenshuai on 2022/10/17.
//
#include "../faces/UltraFace.hpp"
#define clip(x, y) (x < 0 ? 0 : (x > y ? y : x))

#include "UltraFaceNCNN.hpp"
#include "mat.h"
#include "../utils/Log.h"
#include <chrono>
UltraFaceNCNN::UltraFaceNCNN(AAssetManager *mgr_, const char *param_path, const char *bin_path,
...
//    topk = topk_;
    ...
    net->load_param(mgr_, param_path);
    net->load_model(mgr_, bin_path);
}

UltraFaceNCNN::~UltraFaceNCNN() { net->clear(); }

int UltraFaceNCNN::detectNCNN(ncnn::Mat &img, std::vector<FaceInfo> &face_list) {
    if (img.empty()) {
...
//    ncnn::Mat ncnn_img = in;
    ...
    std::vector<FaceInfo> bbox_collection;  // faces
    std::vector<FaceInfo> valid_input;

    auto extractor = net->create_extractor();
    extractor.set_light_mode(true);  // default
    extractor.set_num_threads(num_thread);
    extractor.input("input", ncnn_img);

    ncnn::Mat scores;
    ncnn::Mat boxes;
    extractor.extract("scores", scores);
    extractor.extract("boxes", boxes);
    generateBBox(bbox_collection, scores, boxes);
    nms(bbox_collection, face_list);
    return 0;
}

void UltraFaceNCNN::generateBBox(std::vector<FaceInfo> &bbox_collection, ncnn::Mat scores, ncnn::Mat boxes) {
...
void UltraFaceNCNN::nms(std::vector<FaceInfo> &input, std::vector<FaceInfo> &output, int type) {
